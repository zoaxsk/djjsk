    ...
    async execute(interaction) {
        try {
            ...
            if (allEmojis.length === 0) {
                return interaction.reply({ content: lang.noCustomEmojisFound });
            }
            ...
            collector.on('collect', async i => {
                try {
                    if (i.user.id !== interaction.user.id) {
                        return i.reply({ content: 'This interaction is not for you.', flags: 64 });
                    }
                    ...
                    if (i.customId.startsWith('emoji_')) {
                        ...
                        if (!emoji) {
                            return i.reply({ content: 'Emoji not found.', flags: 64 });
                        }

                        const embed = new EmbedBuilder()
                            .setTitle(lang.selectedEmojiTitle)
                            .setDescription(lang.selectedEmojiDescription.replace('{url}', emoji.url))
                            .setImage(emoji.url)
                            .setColor('#FF00FF');
                    ...
                    if (!i.replied && !i.deferred) {
                        await i.reply({ content: lang.errorProcessingRequest, flags: 64 }).catch(() => {});
                    }
                }
            });
            ...
            if (!interaction.replied) {
                await interaction.reply({ content: lang.errorExecutingCommand }).catch(() => {});
            }
        }
    }
};
